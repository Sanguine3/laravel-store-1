<x-layouts.admin :title="__('Users')">
    <div class="flex flex-col gap-6">
        ...
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4" x-data="{ searchTerm: '{{ $search ?? '' }}' }">
             <div class="flex-1 flex flex-col sm:flex-row sm:items-end gap-4">
                ...
                <!-- Role Filter -->
                <div class="min-w-[160px] md:min-w-[200px] w-full sm:w-auto" x-data="{ openRoleFilter: false, selectedRoleFilter: '{{ $roleFilter ?? '' }}' }">
                    ...
                    <div class="relative">
                        <button @click="openRoleFilter = !openRoleFilter" type="button" id="role-filter-button" class="w-full flex items-center justify-between px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-md shadow-sm bg-white dark:bg-zinc-700 text-sm font-medium text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <span x-text="selectedRoleFilter === '' ? 'All Roles' : selectedRoleFilter.charAt(0).toUpperCase() + selectedRoleFilter.slice(1)">
                                {{ $roleFilter === '' ? 'All Roles' : ucfirst($roleFilter) }}
                            </span>
                            <svg class="ml-2 -mr-0.5 h-4 w-4 text-zinc-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <div x-show="openRoleFilter"
                             ...
                             style="display: none;">
                            <a href="#" @click.prevent="selectedRoleFilter = ''; openRoleFilter = false" class="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-600">All Roles</a>
                            @foreach($roles as $roleOption)
                                <a href="#" @click.prevent="selectedRoleFilter = '{{ $roleOption }}'; openRoleFilter = false" class="block px-4 py-2 text-sm text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-600">{{ ucfirst($roleOption) }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
                    ...
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-400">
                                @if($user->trashed())
                                    <flux:badge size="sm" inset="top bottom" color="danger" class="!bg-red-100 dark:!bg-red-900/30 !text-red-800 dark:!text-red-400">
                                        Deleted
                                    </flux:badge>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end items-center gap-1">
                                    ...
                                    @else
                                        <!-- Restore Form -->
                                        <form action="{{ route('admin.users.restore', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to restore this user?');">
                                            @csrf
                                            @method('PUT') {{-- Or POST, depending on your route definition --}}
                                            <flux:button type="submit" color="success" size="sm" tooltip="Restore" class="!bg-green-500 !text-white hover:!bg-green-600 focus:!ring-green-400">
