        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                            <a href="{{ $sortLink('name') }}" class="flex items-center gap-1 hover:text-zinc-700 dark:hover:text-zinc-200">
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                             <a href="{{ $sortLink('slug') }}" class="flex items-center gap-1 hover:text-zinc-700 dark:hover:text-zinc-200">
                ...
                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-700">
                        ...
                        <tr x-data="{ expanded: false }">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end items-center gap-1"> {{-- Use gap for spacing --}}
                                    <flux:button href="{{ route('admin.categories.edit', $category->id) }}" as="a" color="warning" size="sm" icon="pencil" tooltip="Edit" class="!bg-yellow-400 !text-zinc-900 hover:!bg-yellow-500 focus:!ring-yellow-300" />
                                    <!-- Delete Form -->
                                    <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category? This might affect associated products.');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-zinc-500 dark:text-zinc-400">No categories found matching your criteria.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="p-4 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                @if ($categories->hasPages())
                    <div>
                        {{-- Pagination links automatically include query string params --}}
                        {{ $categories->links() }}
                    </div>
                @endif
            </div>
